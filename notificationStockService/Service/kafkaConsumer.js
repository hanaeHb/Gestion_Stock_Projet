        ...
        eachMessage: async ({ topic, message }) => {
            ...
            // --- CASE 3: FOURNISSEUR VALIDATED ---
                ...
                if (type === "QUOTATION_SUBMITTED") {
                    ...
                    try {
                        // 🔥 🛠️ UPDATE: Beddli 8888/prediction-service l l-Port d Python direct (B7al 5000)
                        // Bla ma d-fouti 3la Gateway, ya3ni 401 ghadi i-m7i completely!
                        ...
                        // 📑 PRINT THE ACTUAL ERROR TO SEE WHAT IS GOING ON IN THE TERMINAL:
                    ...
                    if (nextSupplier) {
                        ...
                        // ===== 🎯 EXECUTE PLAN B CREATION (MATCHING POSTGRES SCHEMA) =====
                        // Gaddin l-payload bash i-koun matches column structure dyal table 'commandes' direct!
                        const newOrderData = {
                            id_commande: String(Date.now()), // Unique string key variable setup for primary key constraint
                        ...
                        try {
                            ...
                            await axios.post("http://5001/api/commandes", newOrderData, {
                            });
